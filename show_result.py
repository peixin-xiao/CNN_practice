...
with tf.Session() as sess  :
    ...
    # sess.run([W_conv1, W_conv2, W_fc, W_out, b_conv1, b_conv2, b_fc,  b_out])
    ...
    y_label = y
    v, y_label = sess.run([v, y_label], feed_dict={x: mnist.test.images, y: mnist.test.labels})
##
sim = np.zeros(shape=[100], dtype=float)
for j in range(100):
    sim[j] = cosine_similarity(v[10], v[j])
# print(sim, '\n', y_label)
# split1, split2 = tf.split(epoch_x, [20, 80], axis=0)
y_ = tf.argmax(y_label, 1)
label2 = np.zeros(shape=[100], dtype=float)
with tf.Session() as sess:
    label1 = sess.run(y_)
for i in range(100):
    if label1[i] == label1[10]:
        label2[i] = sim[i]
##
x_axis = np.arange(1, 101)
plt.plot(x_axis, sim, '_b')
for i in range(100):
    if label2[i] != 0:
        plt.vlines(i+1, 0, label2[i], color="red")


plt.title("image similarity between 10th image and all")
plt.xlabel("image serial")
plt.ylabel("similar degree")
plt.show()
